...
export const dbGetHymns = async (query: string): Promise<Hymn[]> => {
  ...
  const hymns = parseData(
    z.array(HymnSchema),
    res,
    "Error getting hymns from db"
  );
  return hymns;
};
...
export const dbGetHymnById = async (id: string): Promise<Hymn> => {
  ...
  const hymn = parseData(HymnSchema, res, `Error getting hymn "${id} from db"`);
  return hymn;
};
...
export const dbAddHymn = async (name: string): Promise<Hymn> => {
  ...
  const hymn = parseData(HymnSchema, res, `Error adding hymn to db"`);
  return hymn;
};

export const dbDeleteHymn = async (hymnId: string): Promise<void> => {
  const query = `DELETE from hymns WHERE id = $1;`;
